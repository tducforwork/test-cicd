...
class FrontendController extends Controller
{
    ...
    public function index(){
        $pageTitle = __('Manage Frontend Content');
    ...
    public function templatesActive(Request $request)
    {
        ...
        $notify[] = ['success', strtoupper($request->name) . ' ' . __('template activated successfully')];
        return back()->withNotify($notify);
    }

    public function seoEdit()
    {
        $pageTitle = __('SEO Configuration');
    ...
    public function frontendSections($key)
    {
        ...
        return view('admin.frontend.section', compact('section', 'content', 'elements', 'key', 'pageTitle', 'coupons', 'promotions', 'categories'));
    }
    ...
    public function frontendContent(Request $request, $key)
    {
        ...
        foreach ($request->except('_token', 'video') as $inputField => $val) {
            if ($inputField == 'has_image' && $imgJson) {
                foreach ($imgJson as $imgValKey => $imgJsonVal) {
                    $isVideo = str_contains(strtolower($imgValKey), 'video');
                    if ($isVideo) {
                        $validationRule['image_input.' . $imgValKey] = ['nullable', new FileTypeValidate(['mp4', 'mov', 'avi', 'wmv'])];
                    } else {
                        $validationRule['image_input.' . $imgValKey] = ['nullable', new FileTypeValidate(['jpg', 'jpeg', 'png', 'svg'])];
                        $validationMessage['image_input.' . $imgValKey . '.image'] = keyToTitle($imgValKey) . ' ' . __('must be an image');
                    }
                    $validationMessage['image_input.' . $imgValKey . '.mimes'] = keyToTitle($imgValKey) . ' ' . __('file type not supported');
                }
                continue;
        ...
        if (!$request->id && @getPageSections()->$key->element->seo && $type != 'content') {
            $notify[] = ['info',__('Configure SEO content for ranking')];
    ...
    public function frontendElement($key, $id = null)
    {
        ...
        $pageTitle = $section->name . ' ' . __('Items');
        $coupons = \App\Models\Coupon::where('status', 1)->get();
        if ($id) {
            $data = Frontend::where('tempname',activeTemplateName())->findOrFail($id);
            return view('admin.frontend.element', compact('section', 'key', 'pageTitle', 'data', 'coupons'));
        }
        return view('admin.frontend.element', compact('section', 'key', 'pageTitle', 'coupons'));
    }
