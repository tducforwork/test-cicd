...
class ResetPasswordController extends Controller
{
    ...
    */
    ...
    public function showResetForm(Request $request, $token = null)
    {
        ...
        if (SellerPasswordReset::where('token', $token)->where('email', $email)->count() != 1) {
            $notify[] = ['error', 'Invalid token'];
            return redirect()->route('seller.password.request')->withNotify($notify);
        }
        return view('Template::seller.auth.passwords.reset')->with(
    ...
    public function reset(Request $request)
    {
        ...
        $reset = SellerPasswordReset::where('token', $request->token)->orderBy('created_at', 'desc')->first();
        if (!$reset) {
            $notify[] = ['error', 'Invalid verification code'];
            return redirect()->route('seller.login')->withNotify($notify);
        }

        $user = User::seller()->where('email', $reset->email)->first();
