...
class FileManager
{
    ...
    */
	public function upload(){
        ...
        if (!empty($this->path) && !str_starts_with($this->path, '/') && !str_contains($this->path, ':')) {
    ...
    */
	protected function uploadImage(){
        ...
        // Use binary content if real path is unavailable (Plan B for Windows/Tmp issues)
        if (empty($path)) {
            $image = $manager->read($this->file->getContent());
        } else {
	    ...
	    if ($this->thumb) {
            try {
                ...
                if (empty($path)) {
                    $thumbImage = $manager->read($this->file->getContent());
                } else {
                    $thumbImage = $manager->read($path);
                }
                
                $thumbImage->resize($thumb[0], $thumb[1])->save($this->path . '/thumb_' . $this->filename);
            } catch (\Exception $e) {
                // If thumbnail fails, we just don't throw to avoid blocking the main upload
            }
	    }
	}
    ...
    */
	public function makeDirectory($location = null){
        ...
        // Ensure location is absolute if it's relative
        if (!empty($location) && !str_starts_with($location, '/') && !str_contains($location, ':')) {
