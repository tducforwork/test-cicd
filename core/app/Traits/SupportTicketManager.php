...
trait SupportTicketManager
{
    ...
    public function storeSupportTicket(Request $request)
    {
        ...
        if ($request->hasFile('attachments')) {
            ...
            if ($uploadAttachments != 200) {
                if ($this->apiRequest) {
                    $notify[] = __('File could not upload');
        ...
        if ($this->apiRequest) {
            $notify[] = __('Ticket opened successfully');
            return responseSuccess('ticket_open', $notify, [
                'ticket' => $ticket
            ]);
        }

        $notify[] = ['success', __('Ticket opened successfully!')];
    ...
    public function replyTicket(Request $request, $id)
    {
        ...
        if ($this->apiRequest) {
            $notify[] = __('Ticket replied successfully');
    ...
    protected function storeSupportAttachments($messageId)
    {
        ...
        foreach ($this->files as  $file) {
            try {
                ...
                $notify[] = ['error', __('File could not upload')];
                return $notify;
            }
        }
    ...
    public function closeTicket($id)
    {
        ...
        if ($this->apiRequest) {
            $notify[] = __('Ticket closed successfully');
    ...
    public function ticketDownload($attachmentId)
    {
        ...
        if (!$attachment) {
            if ($this->apiRequest) {
                $notify[] = __('Attachment not found');
