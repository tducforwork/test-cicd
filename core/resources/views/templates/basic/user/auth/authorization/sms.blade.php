        ...
        <div class="flex flex-col lg:flex-row gap-6">
            ...
            <div class="flex-1 min-w-0">
                ...
                <div class="flex items-center gap-4 mb-6">
                    ...
                    <h1 class="font-semibold text-[#272343] text-xl md:text-2xl leading-normal">@lang('Xác minh SMS')</h1>
                </div>
                    ...
                    <div class="w-full max-w-md mx-auto">
                        ...
                        <div class="flex justify-center mb-6">
                            <div class="w-20 h-20 bg-[#FFF6F0] rounded-full flex items-center justify-center">
                                <i class="las la-mobile-alt text-4xl text-[#FF6F0F]"></i>
                            </div>
                        </div>

                        <h2 class="w-full font-bold text-[#272343] text-xl leading-[30px] text-center mb-2">@lang('Xác minh số điện thoại')</h2>
                        <p class="text-[#6B7280] text-base leading-6 text-center mb-6">
                            @lang('Mã xác minh 6 chữ số đã được gửi đến số điện thoại của bạn'): +{{ showMobileNumber(auth()->user()->mobileNumber) }}
                        </p>

                        <form action="{{ route('user.verify.mobile') }}" method="POST" class="submit-form ajax-form">
                            @csrf
                        ...
                        <div class="mt-6 text-center">
                            <p class="text-[#6B7280] text-sm">
                                @lang('Nếu bạn không nhận được mã nào'), <span class="countdown-wrapper">@lang('thử lại sau') <span id="countdown" class="fw-bold text-[#FF6F0F]">--</span> @lang('giây')</span> <a href="{{ route('user.send.verify.code', 'sms') }}" class="try-again-link d-none text-[#FF6F0F] hover:underline">@lang('Thử lại')</a>
                            </p>
