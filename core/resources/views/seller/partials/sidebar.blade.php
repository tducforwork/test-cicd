...
<div class="sidebar-user-info">
    ...
    <div style="text-align: center; margin-top: 10px;">
        ...
        <span
            style="font-size: 13px; color: var(--accent); font-weight: 700; display: block;">{{ __($user->shop->name ?? 'Official Store') }}</span>
...
<nav class="sidebar-menu">
    ...
    <a href="{{ route('seller.products.all') }}"
        class="sidebar-link {{ request()->routeIs('seller.products.*') ? 'active' : '' }}">
        <i class="fa-solid fa-boxes-stacked"></i> @lang('Quản lý sản phẩm')
    </a>
    <a href="{{ route('seller.order.all') }}"
        class="sidebar-link {{ request()->routeIs('seller.order.*') ? 'active' : '' }}">
        <i class="fa-solid fa-clipboard-list"></i> @lang('Quản lý đơn hàng')
    </a>
    <a href="{{ route('seller.withdraw.history') }}"
        class="sidebar-link {{ request()->routeIs('seller.withdraw.*') ? 'active' : '' }}">
        <i class="fa-solid fa-wallet"></i> @lang('Lịch sử thanh toán')
    </a>
</nav>
<div class="sidebar-footer" style="padding: 20px; border-top: 1px solid #eee;">
    <a href="{{ route('user.profile.setting') }}" class="btn btn-light w-100"
        style="border: 1px solid var(--border); font-weight: 700; font-size: 12px; display: flex; align-items: center; justify-content: center; gap: 8px;">
