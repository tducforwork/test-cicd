...
<div class="bg-[#F7F7F7]">
    ...
    <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-6">
        <h1 class="text-2xl font-bold text-[#272343]">@lang('Tuyển dụng')</h1>
        <a href="{{ route('seller.jobs.create') }}" class="w-full sm:w-auto inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-[#FF6F0F] text-white font-bold text-sm rounded-[12px] shadow-[inset_0_1px_2px_0_rgba(255,255,255,0.40),inset_0_-1px_2px_0_rgba(0,0,0,0.24),0_1px_2px_0_rgba(0,0,0,0.08)] hover:bg-orange-600 transition-colors">
    ...
    <!-- Search -->
    <form action="{{ route('seller.jobs.index') }}" method="GET" class="mb-6">
        <div class="flex flex-col sm:flex-row items-center gap-3">
            <div class="flex-1 w-full relative">
                <input type="text" name="search" value="{{ request()->search }}" placeholder="@lang('Tìm kiếm tin tuyển dụng...')" 
    ...
    <div class="bg-white rounded-[12px] overflow-hidden">
        ...
        <div class="hidden lg:flex items-center px-4 py-3 bg-[#f1f1f1] rounded-[12px] mb-2">
            <div class="flex-1">
                <span class="text-sm font-semibold text-[#8a8a8a]">@lang('Thông tin công việc')</span>
            </div>
        ...
        <div class="flex flex-col lg:flex-row lg:items-center gap-4 px-4 py-4 lg:py-3 border-b lg:border-none last:border-none lg:hover:bg-gray-50 lg:rounded-[12px] mb-1 group relative">
            ...
            <div class="flex-1 flex items-center gap-3">
                <div class="flex items-center gap-3 min-w-0 w-full">
                    <div class="w-12 h-12 shrink-0 rounded-lg bg-gray-100 flex items-center justify-center text-kviet-orange">
                        <i class="las la-briefcase text-2xl"></i>
                    </div>
                    <div class="flex-1 min-w-0">
                        <a href="{{ route('seller.jobs.edit', ['id' => $job->id, 'type' => 1]) }}" class="text-[15px] lg:text-sm font-bold lg:font-medium text-[#272343] hover:text-[#FF6F0F] transition-colors block lg:truncate">
                            {{ $job->title }}
                        </a>
                        <span class="text-xs text-muted block mt-0.5">
                            <i class="la la-industry"></i> {{ $job->industry?->name ?? 'N/A' }} | 
            ...
            <div class="w-full lg:w-20 mt-2 lg:mt-0 pt-3 lg:pt-0 border-t lg:border-none border-dashed">
                <div class="flex items-center lg:justify-center gap-2">
                    <a href="{{ route('seller.jobs.edit', ['id' => $job->id, 'type' => 1]) }}" class="flex-1 lg:flex-none w-9 h-9 flex items-center justify-center bg-gray-100 rounded-lg hover:bg-[#FF6F0F] hover:text-white transition-colors gap-2 lg:gap-0" title="@lang('Sửa')">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <button type="button" class="flex-1 lg:flex-none w-9 h-9 flex items-center justify-center bg-gray-100 rounded-lg hover:bg-red-500 hover:text-white transition-colors confirmationBtn gap-2 lg:gap-0" 
                        data-question="@lang('Bạn có chắc chắn muốn xóa tin tuyển dụng này?')" 
        ...
        <div class="text-center py-12">
            ...
            <p class="text-gray-500 text-lg">{{ __($emptyMessage ?? 'Chưa có tin tuyển dụng nào') }}</p>
            <a href="{{ route('seller.jobs.create', ['type' => 1]) }}" class="inline-flex items-center gap-2 mt-4 px-5 py-2.5 bg-[#FF6F0F] text-white font-bold text-sm rounded-[12px] hover:bg-orange-600 transition-colors">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4" />
                </svg>
                @lang('Đăng tin tuyển dụng đầu tiên')
            </a>
        </div>
        @endforelse
    </div>
