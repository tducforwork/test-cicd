                                        ...
                                        @foreach ($item as $imgKey => $image)
                                            <div class="col-md-4">
                                                ...
                                                <div class="form-group">
                                                    ...
                                                    <x-image-uploader class="w-100" :imagePath="frontendImage($key, @$content->data_values->$imgKey, @$section->content->images->$imgKey->size)" :image="@$content->data_values->$imgKey" name="image_input[{{ @$imgKey }}]" id="image-upload-input{{ $loop->index }}" :size="$section->content->images->$imgKey->size" :required="false" />
                                                </div>
                                            </div>
                                        @endforeach
                                        ...
                                        @if ($k != 'images')
                                            ...
                                            @elseif($item == 'textarea-nic')
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>{{ __(keyToTitle($k)) }}</label>
                                                        <textarea rows="10" class="form-control ckeditor" name="{{ $k }}">{{ @$content->data_values->$k }}</textarea>
                                                    </div>
                                                </div>
                                            @elseif($k == 'select')
                                                @php
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <select class="form-control" name="{{ $k }}">
                                                            ...
                                                            @foreach($promotions as $promotion)
                                                                <option value="{{ $promotion->id }}" @if (@$content->data_values->$k == $promotion->id) selected @endif>{{ __($promotion->name) }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                            @elseif($k == 'category_ids')
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <label>{{ __(keyToTitle($k)) }}</label>
                                                        <select class="form-control select2-auto-tokenize" name="{{ $k }}[]" multiple>
                                                            @foreach($categories as $category)
                                                                <option value="{{ $category->id }}" @if (in_array($category->id, (array)@$content->data_values->$k)) selected @endif>{{ __($category->name) }}</option>
                                                            @endforeach
                                                        </select>
                                                    </div>
                                                </div>
                                                    ...
                                                    <div class="form-group">
                                                        ...
                                                        <div class="input-group">
                                                            <span class="input-group-text p-0 border-0">
                                                                <input type='text' class="form-control colorPicker" value="{{ @$content->data_values->$k }}">
                                                            </span>
                                                            <input type="text" class="form-control colorCode" name="{{ $k }}" value="{{ @$content->data_values->$k }}">
                                                        </div>
                                                    </div>
                                                </div>
                                                ...
                                                <div class="col-md-12">
                                                    <div class="form-group">
                                                        <div class="alert alert-info" role="alert">
                                                            <i class="las la-info-circle"></i> {{ __($item) }}
                                                        </div>
                                                    </div>
                                                </div>
                                            @else
                                            ...
                                            @if (@$section->element->images)
                                                ...
                                                <td>
                                                    ...
                                                    <div class="user d-flex">
                                                        <div class="thumb me-2 d-flex justify-content-center align-items-center">
                                                            @if(str_contains(strtolower($firstKey), 'video'))
                                                                <i class="la la-play-circle text--primary" style="font-size: 2rem;"></i>
                                                            @else
                            ...
                            @foreach ($section->element as $k => $type)
                                @if ($k != 'modal')
                                    ...
                                    @elseif($type == 'color')
                                        <div class="form-group">
                                            ...
                                            <div class="input-group">
                                                ...
                                                <input type="text" class="form-control colorCode" name="{{ $k }}" value="">
                                            </div>
                                        </div>
                                    @elseif($k == 'instruction')
                                        <div class="form-group">
                                            <div class="alert alert-info" role="alert">
                                                <i class="las la-info-circle"></i> {{ __($type) }}
                                            </div>
                                        </div>
                                    ...
                                    @elseif($type == 'textarea-nic')
                                        <div class="form-group">
                                            <label>{{ __(keyToTitle($k)) }}</label>
                                            <textarea rows="4" class="form-control ckeditor" name="{{ $k }}"></textarea>
                                        </div>
                                    @elseif(is_object($type) || is_array($type))
                                        <div class="form-group">
                                            <label>{{ __(keyToTitle($k)) }}</label>
                                            <select class="form-control" name="{{ $k }}">
                                                @foreach($type as $optsKey => $optsValue)
                                                    <option value="{{ $optsKey }}">{{ __($optsValue) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    @elseif($k == 'promotion_id')
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="{{ $k }}">
                                                <option value="">@lang('Select One')</option>
                                                @foreach($promotions as $promotion)
                                                    <option value="{{ $promotion->id }}">{{ __($promotion->name) }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    @elseif($k == 'coupon_code')
                                        <div class="form-group">
                                            ...
                                            <select class="form-control" name="{{ $k }}">
                                                ...
                                                @foreach($coupons as $coupon)
                                                    <option value="{{ $coupon->coupon_code }}">{{ __($coupon->name) }} ({{ $coupon->coupon_code }})</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    @else
        ...
        (function($) {
            ...
            $(document).on('click','.updateBtn', function() {
                ...
                $.each(obj, function(index, value) {
                    var inputField = modal.find('[name=' + index + ']');
                    if (inputField.length == 0) {
                        inputField = modal.find('[name="image_input[' + index + ']"]');
                    }

                    if (index.toLowerCase().includes('video')) {
                        var parent = inputField.closest('.image--uploader');
                        if (value) {
                            parent.find('.video-preview-overlay').removeClass('d-none');
                            parent.find('.video-name-display').text(value);
                        } else {
                            parent.find('.video-preview-overlay').addClass('d-none');
                        }
                    }
                    ...
                    if (inputField.hasClass('colorCode')) {
                        inputField.parents('.input-group').find('.colorPicker').spectrum("set", value);
                    }
                });
                modal.modal('show');
            });
